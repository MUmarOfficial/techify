...
export const getLessonsByCourse = async (courseId: string): Promise<Lesson[]> => {
  const res = await api.get<ApiResponse<Lesson[]>>(`/lessons/course/${courseId}`);
  return res.data.data;
};

// Public endpoint - no auth required
export const getCourseLessonsPreview = async (courseId: string): Promise<Lesson[]> => {
  const res = await api.get<ApiResponse<Lesson[]>>(`/lessons/course/${courseId}/preview`);
  return res.data.data;
};
